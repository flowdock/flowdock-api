...
describe FlowdockApi do
  describe "with initializing flow" do
    it "should succeed with correct token and sender information" do
      lambda {
        @flow = FlowdockApi.new(:api_token => "test", :source => "myapp", 
    ...
    it "should fail without token" do
      lambda {
        @flow = FlowdockApi.new(:api_token => "", :source => "myapp", 
          :from => {:name => "test", :address => "invalid@nodeta.fi"})
      }.should raise_error(FlowdockApi::Flow::ApiTokenMissingError)
    end
  
    it "should fail without source" do
      lambda {
        @flow = FlowdockApi.new(:api_token => "test", :source => "",
         :from => {:name => "test", :address => "invalid@nodeta.fi"})
      }.should raise_error(FlowdockApi::Flow::InvalidSourceError)
    end
  
    it "should fail when source is not alphanumeric" do
      lambda {
        @flow = FlowdockApi.new(:api_token => "test", :source => "$foobar",
         :from => {:name => "test", :address => "invalid@nodeta.fi"})
      }.should raise_error(FlowdockApi::Flow::InvalidSourceError)
    end
    ...
    it "should fail without sender information" do
      ...
      }.should raise_error(FlowdockApi::Flow::InvalidSenderInformationError)
